...
export default class DragMoveModel {
  ...
  constructor(config = {} as DragMoveConfig, callback = () => { }) {
    ...
    this.initEvent();
    this.initTragetElInfo();
    this.callback = callback;
  }

  // 初始化配置
  private initConfig(config: DragMoveConfig) {
  ...
  private getStyleTransformProp(transform: string = '', prop = 'scale') {
    transform = transform.replace(/, /g, ',').trim();
    const strArr = transform.split(' ');
    let res = '';
    strArr.forEach((str) => {
  ...
  private calcTargetTranlate = () => {
    if (this.targetEl) {
      let translate = this.getStyleTransformProp(this.targetEl.style.transform, 'translate3d');
      if (translate.indexOf('translate3d') > -1) {
        const reg = /\((.*)\)/g;
        const res = reg.exec(translate);
        if (res) {
          translate = res[1].replace(/, /g, ',');
        }
        const translateArr = translate.replace('(', '').replace(')', '').split(',');
  ...
  // 设置transform属性
  private setTransformProp(transform = '', prop = '', value = '') {
    const reg = new RegExp(`${prop}\((.*)\)`, 'g');
    if (transform.indexOf(prop) > -1) {
      const propList = transform.replace(/, /g, ',').trim().split(' ');
      const newPropList = propList.map((item) => item.replace(reg, `${prop}(${value})`));
  ...
  private translateMoveEl() {
    if (this.targetEl) {
      ...
      let { transform } = this.targetEl.style;
      transform = transform ? this.setTransformProp(transform, 'translate3d', `${tx}px, ${ty}px, 0px`) : `translate3d(${tx}px, ${ty}px, 0px)`;
    ...
    if (this.isMousedown) {
      ...
      if (!this.disableMoveEl) {
        if (this.moveMode === 'position') {
          this.topLeftMoveTargetEl();
        } else {
          this.translateMoveEl();
        }
      }

      // 计算第三边的长度（勾股定理 a^2 + b^2 = c^2）
      const c = Math.round((this.moveInsX * this.moveInsX + this.moveInsY * this.moveInsY) ** 0.5);
  ...
  private initEvent() {
    ...
    this.rootDom.addEventListener(moveEvent, this.mousemoveHandler);
    if (this.targetEl) {
      this.targetEl.addEventListener(downEvent, this.mousedownHandler);
    }
    this.rootDom.addEventListener(upEvent, this.mouseupHandler);
  }
  ...
  destroy() {
    ...
    const upEvent = this.h5 ? 'touchend' : 'mouseup';
    if (this.targetEl) {
      this.targetEl.removeEventListener(moveEvent, this.mousedownHandler);
    }
    this.rootDom.removeEventListener(downEvent, this.mousemoveHandler);
    this.rootDom.removeEventListener(upEvent, this.mouseupHandler);
  }
}
